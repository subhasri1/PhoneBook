...
void transact_with_server(int sd,int argc,char*argv[])
{
	...
	else if(argc==3) // send name to be deleted
	{	char buf[100],msg[10];
	
		int n =send(sd,argv[1],strlen(argv[1]),0);// send -r option	
	...
	else //send phone number to be deleted
	{
		int n =send(sd,argv[2],strlen(argv[2]),0);// send -n option
		n=send(sd,argv[3],strlen(argv[3]),0);//send number
